...
"""
Test MC-PILCO on cartpole — Variant J (per-particle GP-vs-GFN reverse KL).

Variant J is the first cost that uses the EXACT per-particle GP output
Gaussian rather than collapsing all particles into a single moment-matched
Gaussian. At each timestep t and particle m:

    KL_m(t) = KL( p_GFN || N(mu_GP_m(t), sigma^2_GP_m(t)) )

Both sides are true Gaussians (GP posterior and GFN target), so the KL
is exact -- no moment matching, no sign correction.

Requires MC_PILCO_GPStats (not plain MC_PILCO), which captures the GP
means and variances during the rollout and stores them on the cost object.

Everything else (GP model, policy, safety constraints, hyperparameters)
is identical to Variant C for a clean comparison.
"""
...
# CLI
p = argparse.ArgumentParser("test cartpole variant J (per-particle GP-vs-GFN KL)")
...
# ---------------------------------------------------------------------------
# Model learning (identical to Variant C)
# ---------------------------------------------------------------------------
...
from policy_learning.variant_j_cost import VariantJ_Cost
f_cost_function = VariantJ_Cost
...
# ---------------------------------------------------------------------------
# Build MC_PILCO_GPStats (NOT plain MC_PILCO — needed to pass GP stats)
# ---------------------------------------------------------------------------
print("\n---- Init policy learning object (MC_PILCO_GPStats) ----")
results_dir = f"results_variant_j/{seed}"
...
# ---------------------------------------------------------------------------
# MC-PILCO options (identical to Variant C)
# ---------------------------------------------------------------------------
