...
"""
Test MC-PILCO on UR5 trajectory tracking -- Variant I.

LOCAL per-step Jensen-Shannon Divergence with time-varying target.
...
JSD properties vs Mahalanobis / KL:
...
Key differences from Variant H:
  * Cost: per-step JSD(q_k || p_k) estimated via Monte Carlo, where
    q_k is fitted from rollout particles and p_k = N(mu_ref(t_k), sigma_p^2).
  * n_p_samples parameter controls variance of gradient estimate for
    KL(p||M) term. Default 50.
...
Usage:
    ...
    python test_mcpilco_ur5_variant_i.py -seed 1 -num_trials 5

    # Resume an interrupted run (default behaviour)
    python test_mcpilco_ur5_variant_i.py -seed 1 -num_trials 5

    # 1-step lookahead policy (no BPTT through GP chain)
    python test_mcpilco_ur5_variant_i.py -seed 1 -num_trials 5 --bptt 1

    # Truncated BPTT every 10 steps
    python test_mcpilco_ur5_variant_i.py -seed 1 -num_trials 5 --bptt 10
"""
...
# CLI
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser("test ur5 variant I (local per-step JSD)")
...
parser.add_argument("-results_root", type=str, default="results_ur5_variant_i",
...
print(f"[variant_i] initial state = zeros (aligned with GFN diffusion prior)")
print(f"[variant_i] ||zeros - q_ref[0]|| = {_d_start_to_ref0:.3f} rad "
...
# ---------------------------------------------------------------------------
# Cost (Variant I: per-step JSD)
# ---------------------------------------------------------------------------
print(f"\n---- Set cost function (Variant I: per-step local JSD) ----")
from policy_learning.ur5_variant_i_cost import UR5_VariantI_Cost
f_cost_function = UR5_VariantI_Cost
...
print(f"\n[variant_i] CWD            : {pathlib.Path.cwd()}")
print(f"[variant_i] log_dir (rel)  : {log_dir}")
print(f"[variant_i] log_dir (abs)  : {log_dir_abs}")
print(f"[variant_i] log_file (abs) : {log_file_abs}")
...
print("[variant_i] safe-save patch active (atomic write + flush + fsync).")
...
print(f"[variant_i] Replaced PL_obj.system with UR5_Model (MuJoCo, "
...
# Run
# ---------------------------------------------------------------------------
print(f"\n[variant_i] Starting reinforce() with "
...
print(f"\n[variant_i] Done. Log saved to {log_file_abs}")
