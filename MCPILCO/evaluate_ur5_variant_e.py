"""
Evaluation script for MC-PILCO UR5 Variant E (per-particle cross-entropy) results.

Usage:
    python evaluate_ur5_variant_e.py                         # uses results_ur5_variant_e_5/1/log.pkl
    python evaluate_ur5_variant_e.py -log .../log.pkl
    python evaluate_ur5_variant_e.py -no_anim                # skip GIF (faster)
...
Cost is per-particle cross-entropy under the time-varying GFN target
plus chance-constraint slack on joint position bounds. The mean term
operates on individual particles (no moment fitting), so it cannot be
fooled by "wide cloud with right mean" cheats that broke Variants C/D.
"""
...
# CLI
# -----------------------------------------------------------------------
p = argparse.ArgumentParser("evaluate_ur5_variant_e")
p.add_argument("-log", default="results_ur5_variant_e_5/1/log.pkl")
p.add_argument("-save", default="results_ur5_variant_e_5/1/plots")
...
fig.suptitle("UR5 Variant E - policy optimisation cost per trial",
...
# -----------------------------------------------------------------------
print("\n" + "=" * 78)
print("EVALUATION SUMMARY — MC-PILCO UR5 Variant E")
print("=" * 78)
