...
"""
Test MC-PILCO on the UR5 trajectory-tracking task — Variant C (reverse KL).
...
This is the diffusion-time -> physical-time conversion: k/N_h in [0,1]
maps to physical_time = (k/N_h) * T_control.
"""
...
# CLI
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser("test ur5 variant C (reverse KL, time-varying target)")
...
# ---------------------------------------------------------------------------
print("\n---- Set cost function (Variant C: reverse KL + relaxed safety) ----")
from policy_learning.ur5_variant_c_cost import UR5_VariantC_Cost
f_cost_function = UR5_VariantC_Cost
cost_function_par = {
...
MC_PILCO_init_dict = {
    ...
    "log_path":                   f"results_ur5_variant_c_5/{seed}",
    "dtype":                      dtype,
    "device":                     device,
}
...
log_dir = pathlib.Path("results_ur5_variant_c_5") / str(seed)
