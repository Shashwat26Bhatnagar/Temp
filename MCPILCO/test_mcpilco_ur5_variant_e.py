...
"""
Test MC-PILCO on UR5 trajectory tracking - Variant E
(per-particle cross-entropy with time-varying GFN target).
...
For a diagonal Gaussian GFN target the per-particle negative log-density is
    -log p_GFN(x_i) = 0.5 * sum_d (x_{i,d} - mu_{p,d})^2 / sigma_{p,d}^2
                      + const

Compared to Variant D (W2 on fitted moments):
  * Each particle contributes its OWN distance -- no moment fitting.
  * Cannot be fooled by "wide cloud whose mean matches target" cheat.
  * Cheaper to compute (no fitting, no W2 closed form).
  * GFN's sigma_p still used as per-dimension precision weighting.
...
# CLI
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser("test ur5 variant E (per-particle cross-entropy)")
...
# ---------------------------------------------------------------------------
print("\n---- Set cost function (Variant E: per-particle cross-entropy) ----")
from policy_learning.ur5_variant_e_cost import UR5_VariantE_Cost
f_cost_function = UR5_VariantE_Cost
cost_function_par = {
...
MC_PILCO_init_dict = {
    ...
    "log_path":                   f"results_ur5_variant_e_5/{seed}",
    "dtype":                      dtype,
    "device":                     device,
}
...
log_dir = pathlib.Path("results_ur5_variant_e_5") / str(seed)
