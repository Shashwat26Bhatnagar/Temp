"""
Evaluation script for MC-PILCO UR5 Variant I (local per-step JSD).
...
Usage:
    python evaluate_ur5_variant_i.py                          # default log dir
    python evaluate_ur5_variant_i.py -log <path>/log.pkl
    python evaluate_ur5_variant_i.py -no_anim                 # skip GIF
    python evaluate_ur5_variant_i.py -trial 2                 # plot a specific rollout
...
This is the JSD-based analog of Variant H's evaluator. Targets are
TIME-VARYING here (q_ref(t)), same as Variant H, but the cost metric
is Jensen-Shannon Divergence rather than Mahalanobis distance.
"""
...
# CLI
# ---------------------------------------------------------------------------
p = argparse.ArgumentParser("evaluate_ur5_variant_i")
p.add_argument("-log", default="results_ur5_variant_i/1/log.pkl")
...
if not log_path.exists():
    raise SystemExit(f"[evaluate_i] log file not found: {log_path}\n"
...
# ---------------------------------------------------------------------------
# Start/End alignment check between Variant I rollout and GFN diffusion
# ---------------------------------------------------------------------------
...
if _cfg_path.exists():
    try:
        ...
        _i_initial_q  = _init_state[:6]
        _i_initial_dq = _init_state[6:]
...
if _i_initial_q is None:
    _i_initial_q  = q_ref[0]
    _i_initial_dq = dq_ref[0]
    print(f"[align] no config_log.pkl -- assuming initial state = q_ref[0]")

print("\n" + "=" * 78)
print("START / END ALIGNMENT CHECK   (Variant I vs GFN diffusion)")
print("=" * 78)
print(f"MC-PILCO I initial state (q)  :  "
      f"[{', '.join(f'{x:+.4f}' for x in _i_initial_q)}]")
print(f"MC-PILCO I terminal target q_ref[-1]:  "
...
if gfn_mu_t is not None:
    ...
    d_start_i_vs_gfn = float(np.linalg.norm(_i_initial_q - 0.0))
    d_end_i_vs_gfn   = float(np.linalg.norm(q_ref[-1] - gfn_mu_t[:6]))
    d_circle_close   = float(np.linalg.norm(q_ref[0] - q_ref[-1]))
    d_start_to_ref0  = float(np.linalg.norm(_i_initial_q - q_ref[0]))
    print(f"\n||MC-PILCO I init - GFN_start (zeros)|| = {d_start_i_vs_gfn:.4f} rad "
          f"({'aligned (Option B: both start at zeros)' if d_start_i_vs_gfn < 0.05 else 'MISMATCH (I init is NOT zeros)'})")
    print(f"||q_ref[-1]       - GFN_mu_t        || = {d_end_i_vs_gfn:.4f} rad "
          f"({'aligned (circle endpoint == GFN target)' if d_end_i_vs_gfn < 0.05 else 'MISMATCH'})")
    print(f"||q_ref[0]        - q_ref[-1]       || = {d_circle_close:.4f} rad "
          f"({'closed loop' if d_circle_close < 0.05 else 'OPEN loop'})")
    print(f"||MC-PILCO I init - q_ref[0]        || = {d_start_to_ref0:.4f} rad "
...
if n_hist == 0:
    print("[evaluate_i] No real rollouts in log yet -- nothing to plot.")
    sys.exit(0)
...
if n_trials > 0:
    ...
    fig.suptitle("UR5 Variant I -- policy optimisation cost per trial",
...
axes = axes.flatten()
fig.suptitle(f"UR5 Variant I -- joint positions (rollout {trial_idx})  "
...
fig, axes = plt.subplots(2, 3, figsize=(15, 8), sharex=True)
axes = axes.flatten()
fig.suptitle(f"UR5 Variant I -- joint velocities (rollout {trial_idx})  "
...
ax.set_title(f"End-effector 3D trajectory (Variant I, rollout {trial_idx})")
...
if not args.no_anim:
    ...
    title_obj   = ax.set_title(f"UR5 Variant I rollout {trial_idx}  (t = 0.00 s)",
                               fontsize=11)
    ax.set_xlabel("X (m)"); ax.set_ylabel("Y (m)"); ax.set_zlabel("Z (m)")
    ...
    def update(fnum):
        ...
        title_obj.set_text(
            f"UR5 Variant I rollout {trial_idx}  (t = {t_idx * T_sampling:.2f} s)")
        return arm_line, trail_line, ee_dot, title_obj
...
# ---------------------------------------------------------------------------
print("\n" + "=" * 78)
print("EVALUATION SUMMARY -- MC-PILCO UR5 Variant I")
print("=" * 78)
