...
"""
Test MC-PILCO on UR5 trajectory tracking - Variant F
(REVERSE KL on fitted moments with tight target sigma).
...
Variant F is the reverse-KL counterpart of D (W2) and E (forward KL)
with the same tight target sigma (0.05 rad / 0.20 rad/s). This isolates
the KL direction as the variable across UR5 experiments.

PREDICTION (recorded a-priori for falsification):
Variant F is expected to FAIL on UR5 tracking due to the reverse-KL
pathology with tight sigma_p:
  * Reverse KL has sigma_q in the denominator (trace term sigma_p^2 / sigma_q^2)
  * When the policy tracks well, sigma_q is tiny (particles concentrate)
  * Trace term blows up when sigma_q << sigma_p
  * The cost ACTIVELY PUNISHES tight tracking
  * Optimizer prefers wide particles (variance inflation), which prevents
    accurate tracking
...
# CLI
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser("test ur5 variant F (reverse KL with tight sigma)")
...
MC_PILCO_init_dict = {
    ...
    "log_path":                   f"results_ur5_variant_f_5/{seed}",
    "dtype":                      dtype,
    "device":                     device,
}
...
log_dir = pathlib.Path("results_ur5_variant_f_5") / str(seed)
