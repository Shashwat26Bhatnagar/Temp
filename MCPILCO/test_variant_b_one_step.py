...
"""
Smoke test: MC-PILCO with VariantB_Cost — 3 optimization steps only.
"""
...
num_trials = 1  # SMOKE TEST: only 1 trial
T_sampling = 0.05  # Sampling time
T_exploration = 3.0  # Duration of the first exploration trial
T_control = 1.0  # SMOKE TEST: shorter horizon to avoid GP variance blowup
...
MC_PILCO_init_dict["log_path"] = "results_smoke_test/" + str(seed)  # path to save logs of the experiments
...
model_optimization_opt_dict["N_epoch"] = 500  # SMOKE TEST: quick GP fit
model_optimization_opt_dict["N_epoch_print"] = 100  # SMOKE TEST: print more often
...
policy_optimization_dict["num_particles"] = 400  # SMOKE TEST: keep standard count to avoid GP variance blowup
policy_optimization_dict["opt_steps_list"] = [2]  # SMOKE TEST: only 3 optimization steps
policy_optimization_dict["lr_list"] = [0.01]  # SMOKE TEST: single trial
policy_optimization_dict["f_optimizer"] = "lambda p, lr : torch.optim.Adam(p, lr)"  # Specify policy optimizer
policy_optimization_dict["num_step_print"] = 1  # SMOKE TEST: print every step
policy_optimization_dict["p_dropout_list"] = [0.25]  # SMOKE TEST: single trial
...
policy_optimization_dict["num_min_diff_cost"] = 1  # SMOKE TEST: reduced
policy_optimization_dict["min_step"] = 1  # SMOKE TEST: reduced
...
import os
os.makedirs("results_smoke_test/" + str(seed), exist_ok=True)
...
pkl.dump(config_log_dict, open("results_smoke_test/" + str(seed) + "/config_log.pkl", "wb"))

# Start the learning algorithm
try:
    print("\n=== Starting smoke test: 3 optimization steps ===")
