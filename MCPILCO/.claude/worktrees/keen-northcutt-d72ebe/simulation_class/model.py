...
class Model:
    ...
    def __init__(self, fcn):
        ...
        """
        # Wrap fcn to ensure it returns a proper 1D numpy array for odeint
        def fcn_wrapper(x, t, u):
            result = fcn(x, t, u)
            # Aggressively flatten any nested structure
            if hasattr(result, '__iter__'):
                flat_result = []
                for item in result:
                    if hasattr(item, '__iter__') and not isinstance(item, str):
                        flat_result.extend(np.atleast_1d(item).flatten())
                    else:
                        flat_result.append(float(item))
                return np.array(flat_result, dtype=np.float64)
            else:
                return np.array([float(result)], dtype=np.float64)
        self.fcn = fcn_wrapper  # ODE of system dynamics
