"""
UR5 Variant D cost -- 2-Wasserstein distance with time-varying target trajectory.

    L = sum_t  w_t * W2^2( p_GFN(t), q_t )  +  alpha * sum_t  slack_t

where
    q_t       = diagonal Gaussian fitted to particle moments at step t
    p_GFN(t)  = N( mu_p(t), diag(sigma_p^2) )
                with mu_p(t) sliding along the reference trajectory
                (diffusion -> physical time conversion).
    w_t       = (t / N_h)^2  (quadratic time weighting)
    slack_t   = soft chance-constraint slack on joint positions

W2^2 (squared 2-Wasserstein) for diagonal Gaussians has the closed form

    W2^2(p, q) = || mu_p - mu_q ||^2 + || sigma_p - sigma_q ||^2

This has structural advantages over reverse KL for trajectory tracking:
  * The mean term is EXACTLY pointwise tracking distance -- the same signal
    standard MC-PILCO uses. No "wide-cloud cheat".
  * Symmetric -- no forward/reverse choice to make.
  * Numerically stable -- no sigma in denominator, no NaN guards needed.
  * Cheaper to compute -- no log, no division.

The hypothesis (per our analysis): switching from reverse KL to W2^2 should
substantially fix the UR5 trajectory-tracking failure where the optimizer
minimised the KL cost while task error grew.
"""
...
from policy_learning.kl_cost import (
    ...
    wasserstein2_squared_gaussian_diag,
)


class UR5_VariantD_Cost:
    """
    UR5 cost driven by the squared 2-Wasserstein between the time-varying
    ...
    def __init__(self,
        ...
        # For tracking tasks, the GFN's terminal-state sigma (0.10 / 0.50)
        # may be too loose -- it lets the optimiser tolerate ~0.1 rad
        # tracking errors. Allow tightening here.
        ...
        # Logging buffers
        self.last_w2_per_step    = None
        self.last_slack_per_step = None
        self.last_weights        = None
        self.last_mu_p_traj      = None

        print(f"[UR5_VariantD_Cost] W2-squared + chance constraints | "
              f"alpha={alpha} epsilon={epsilon} weighting='{weighting}'")
        print(f"[UR5_VariantD_Cost] target sigma_q={self.sigma_p[:6].tolist()}")
        print(f"[UR5_VariantD_Cost] target sigma_dq={self.sigma_p[6:].tolist()}")
        print(f"[UR5_VariantD_Cost] q_min={list(q_min)}")
        print(f"[UR5_VariantD_Cost] q_max={list(q_max)}")
    ...
    def cost_function(self, states_sequence, inputs_sequence, trial_index):
        ...
        # Sanitise runaway particles (paranoia: W2 is stable but particles
        # from GP rollout can still go NaN/inf in early trials).
        ...
        # ============================================================
        # 1) Per-step W2^2 with TIME-VARYING target mean
        # ============================================================
        ...
        w2_sq_per_step = wasserstein2_squared_gaussian_diag(
        ...
        weighted_w2_per_step = weights * w2_sq_per_step
        ...
        # ============================================================
        cost_per_step = weighted_w2_per_step + self.alpha * slack_per_step

        # Final guard (W2 itself is well-behaved, but the GP rollout
        # earlier could have produced extreme values).
        cost_per_step = torch.nan_to_num(cost_per_step,
                                         nan=1e8, posinf=1e8, neginf=0.0)
        cost_per_step = torch.clamp(cost_per_step, max=1e8)

        # Logging
        self.last_w2_per_step    = w2_sq_per_step.detach()
